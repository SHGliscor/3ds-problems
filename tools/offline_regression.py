...
def test_honey_duplicate_rows_are_not_a_false_preflight_hold():
    class FakeBridge:
        def __init__(self):
            self.mem = {}
    ...
    br = FakeBridge()
    items = bytearray(ORAS_ITEMS_SIZE)
    # Simulate the false-hold state: multiple populated Honey rows, with Honey
    # still the first active Items entry.  Preflight must accept this state.
    rows = [
        (HONEY_ITEM_ID, 10),
        (17, 4),
        (HONEY_ITEM_ID, 5),
    ...
    for save_index, (item_id, qty) in enumerate(rows):
        struct.pack_into('<HH', items, save_index * 4, item_id, qty)
    br.put(ORAS_ITEMS_ADDR, items)

    honey = require_honey(br, first_active_slot=True)
    assert honey['item_id'] == HONEY_ITEM_ID
    assert honey['active_index'] == 0

    authority = {
        'entries': [
            {'index': 0, 'item_id': HONEY_ITEM_ID, 'quantity': 10},
            {'index': 1, 'item_id': 17, 'quantity': 4},
            {'index': 2, 'item_id': HONEY_ITEM_ID, 'quantity': 5},
        ]
    }
    assert honey_active_index(authority) == 0


def test_wild_run_post_ceiling_late_inactive_race_fix():
    worker = (ROOT / "qt_ui" / "wild_worker.py").read_text(encoding="utf-8")
    expect("handoff_battle = br.u32(core.BATTLE_ADDR)" in worker,
           "wild Run wrapper must re-read battle state at the post-ceiling handoff")
    expect("battle left active during post-ceiling handoff" in worker,
           "late inactive Run transition must be treated as a valid escape handoff")
    expect("core.wait_field_stable(br, core.FIELD_RETURN_TIMEOUT_SEC)" in worker,
           "late inactive Run transition must still require stable field authority")
    expect('"accepted_attempt": attempts[-1].get("attempt") if attempts else None' in worker,
           "late inactive Run transition must preserve the final accepted Run attempt")
...
def main():
    tests = [
        ...
        test_honey_duplicate_rows_are_not_a_false_preflight_hold,
        test_wild_run_post_ceiling_late_inactive_race_fix,
