...
def require_honey(br, *, first_active_slot: bool = False) -> dict:
    ...
    if not honey:
        raise RuntimeError(
            "HONEY PREFLIGHT: no live Items-pocket Honey entry (item 94) was found"
        )
    # Some ORAS save/RAM states can expose more than one populated Honey row.
    # Multiple rows are not, by themselves, proof that an unsafe item will be
    # selected: the fast path independently requires the first active entry to
    # be item 94 and the live DllBag controller must later prove the same exact
    # active inventory.  Treat all Honey rows as valid stock and select the
    # first active Honey row rather than issuing a false preflight hold.
    row = dict(honey[0])
...
def honey_active_index(authority: dict) -> int:
    ...
    if not rows:
        raise RuntimeError("FIELD BAG: no live Honey entry remains in the controller")
    # Duplicate Honey rows are permitted by the tolerant preflight.  The
    # controller still proves the exact active inventory and the first Honey
    # entry is the only one eligible for the slot-1 fast path.
    for index, row in enumerate(authority["entries"]):
        if int(row["item_id"]) == HONEY_ITEM_ID and int(row["quantity"]) > 0:
            return int(index)
