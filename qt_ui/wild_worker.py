...
class WildHuntWorker(QObject):
    ...
    def _causal_run_until_field(self, br, core):
        ...
        try:
            handoff_battle = br.u32(core.BATTLE_ADDR)
        except Exception:
            return causal

        # Race fix: the game can leave ACTIVE immediately after the core's final
        # 0.30 s observation window expires.  In that case the old code returned
        # the failed 24-tap result even though battle RAM had already transitioned
        # to INACTIVE.  Re-enter the proven field-stability authority path instead
        # of converting that late, valid transition into a false Safety Hold.
        if handoff_battle != core.BATTLE_ACTIVE:
            if handoff_battle == core.BATTLE_INACTIVE:
                field = core.wait_field_stable(br, core.FIELD_RETURN_TIMEOUT_SEC)
                return {
                    "success": bool(field.get("stable")),
                    "accepted_attempt": attempts[-1].get("attempt") if attempts else None,
                    "attempts": attempts,
                    "field": field,
                    "first_nonactive": core.hx(handoff_battle),
                    "note": "battle left active during post-ceiling handoff",
                    "run_recovery_extension_used": False,
                }
            return causal
    ...
    def _honey_quantity(snapshot):
        ...
        # Honey may occupy more than one populated save row in some live
        # inventory states.  Quantity authority is the total live Honey stock.
        # A2/retry safety still requires the total to change by exactly one.
        if not rows:
            return None
